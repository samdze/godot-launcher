...
onready var hotspots_container = $PanelContainer/HotspotsContainer
...
func _focus():
	...
	emit_signal("title_change_requested", "Wi-Fi Settings")
	emit_signal("mode_change_requested", LauncherUI.Mode.OPAQUE)
	Launcher.get_ui().bottom_bar.set_prompts([BottomBar.ICON_NAV_V, BottomBar.PROMPT_NAV], [BottomBar.ICON_BUTTON_A, BottomBar.PROMPT_SELECT, BottomBar.ICON_BUTTON_Y, BottomBar.PROMPT_SCAN, BottomBar.ICON_BUTTON_B, BottomBar.PROMPT_BACK])
...
func _hotspot_pressed(network_details : NetworkDetails):
	if network_details.encryption == Encryption.OFF:
		...
		var keyboard = Modules.get_loaded_component_from_config("system", "keyboard_app", "default/keyboard").resource.instance()
		keyboard.title = "Wi-Fi Password"
...
func _connection_attempted(result):
	...
	if result == 0:
		Launcher.get_ui().notifications_bar.append_notification("Successfully connected to " + connecting_network.name + "!", Notifications.Type.SUCCESS)
	else:
		Launcher.get_ui().notifications_bar.append_notification("Failed to connect to " + connecting_network.name + ".", Notifications.Type.ERROR)
